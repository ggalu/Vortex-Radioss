...
# @Date:   2025-09-21 13:11:39
# @Last Modified by:   Georg C. Ganzenmueller, Albert-Ludwigs Universitaet Freiburg, Germany
# @Last Modified time: 2025-09-21 14:14:43
...
class convert:
    ...
    @staticmethod
    def element_shell_stress_axissymmetric(*data):
        # create a full stress tensor using the 6 separate stress arrays in Voigt notation
        # requested by /ANIM/ELEM/SIGX,  /ANIM/ELEM/SIGY,  /ANIM/ELEM/SIGZ
        #              /ANIM/ELEM/SIGXY, /ANIM/ELEM/SIGYZ, /ANIM/ELEM/SIGZX.
        # This stress tensor is copied to both integration points

        #console.log("building axis-symmetric stres tensor")
        #console.log(f"len of data {len(data)}")

        shell_num = len(data[0])
        nip = data[6][0]
        #console.log(f"number of integration points {nip}")

        out = np.zeros((shell_num, nip, 6))

        for ip in range(nip):
            out[:,ip,0] = data[0] # ["element_shell_stress_x_"]
            out[:,ip,1] = data[1] # ["element_shell_stress_y_"]
            out[:,ip,2] = data[2] # ["element_shell_stress_z_"]
            out[:,ip,3] = data[3] # ["element_shell_stress_xy"]
            out[:,ip,4] = data[4] # ["element_shell_stress_yz"]
            out[:,ip,5] = data[5] # ["element_shell_stress_zx"]
...
class readAndConvert:
    ...
    def __init__(
        ...
        self._d3plot = D3plot()
        self.A_2_D(filepath, use_shell_mask, use_solid_mask, use_beam_mask, use_axissymmetric, silent, no_warnings)
    ...
    def A_2_D(self,file_stem, use_shell_mask, use_solid_mask, use_beam_mask, use_axissymmetric, silent, no_warnings):
        ...
        console.log("array keys in Anim file:")
        for key in rr.arrays.keys():
            console.log(key)
        ...
        for ifile, file in enumerate(tqdm(file_list, disable = silent)):            
            ...
            if True:
                ...
                if n_shell > 0:   
                   ...
                   # Dyna output
                    ...
                    # Dyna output
                    if use_axissymmetric:
                        # ArrayType.element_shell_stress is not defined in the axissymmetric case.
                        # Instead, rr.arrays["element_shell_stress..."], requested by /ANIM/ELEM/SIGX ... is used to build
                        # ArrayType.element_shell_stress
                        # a custom conversion routine is called for axis-symmetric stress
                        array_requirements[ArrayType.element_shell_stress] = {}
                        _ = array_requirements[ArrayType.element_shell_stress]
                        # Radioss outputs needed to comptute Dyna output
                        _["dependents"]     = ["element_shell_stress_x_",
                                               "element_shell_stress_y_",
                                               "element_shell_stress_z_",
                                               "element_shell_stress_xy",
                                               "element_shell_stress_yz",
                                               "element_shell_stress_zx",]
                        _["shape"]          = (1,n_shell, nip_shell, 6)
                        _["convert"]        = convert.element_shell_stress_axissymmetric
                        _["tracker"]        = shell_ids_tracker
                   ...
                   # Dyna output
                    ...
                    if use_axissymmetric:
                        # ArrayType.element_shell_effective_plastic_strain is not defined in the axissymmetric case.
                        # Instead, rr.arrays["element_shell_plastic_strain"], requested by /ANIM/ELEM/EPSP is used to build
                        # ArrayType.element_shell_effective_plastic_strain
                        array_requirements[ArrayType.element_shell_effective_plastic_strain] = {}
                        _ = array_requirements[ArrayType.element_shell_effective_plastic_strain]
                        # Radioss outputs needed to comptute Dyna output
                        _["dependents"]     = ["element_shell_plastic_strain", 'element_shell_plastic_strain']
                        _["shape"]          = (1, n_shell, nip_shell)
                        _["convert"]        = convert.element_shell_effective_plastic_strain
                        _["tracker"]        = shell_ids_tracker  
                        _["additional"]     = [nip_shell]
            ...
            for flag in database_extent_binary:
                ...
                for array_group in _:
                    ...
                    #console.log(f"processing {__[0]}, full: {__}")
                    
                    if array_group in flag_max[flag]:
                        #console.log(f"... this array_group is present in flag_max[flag] = {flag_max[flag]}" )
                        for array_output in __:
                            if dependency_check[array_output]:
                                conversion_function = array_requirements[array_output]["convert"]
                                #console.log("conversion function:", conversion_function)
